...
export const Header: React.FC = () => {
  ...
  const { spaces, currentSpace, requestSpaceSwitch } = useWatchSpace();
            ...
            {spaces.length > 0 && (
              <div className="relative">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold hover:bg-slate-800/60 border ${
                    currentSpace?.isKids
                ...
                >
                  ...
                  {currentSpace?.isKids && (
                    <span className="px-1.5 py-0.2 rounded text-[10px] bg-amber-500/20 text-amber-300 font-bold">Kids</span>
                  )}
                  <ChevronDown className="w-3.5 h-3.5 opacity-70" />
                </button>
                {showSpaceMenu && (
                  <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden z-50">
                    ...
                    {spaces.map((s) => (
                      ...
                      >
                        <span className="truncate flex items-center gap-1.5">
                          {s.isKids ? '👶' : <Users className="w-3 h-3 text-slate-400" />}
                          {s.name}
                        </span>
                        {s.isKids && (
